...
//进入出差 (当前仅有一个未登记或执行中出差)
@SuppressWarnings("serial")
public class EnterBussinessServlet extends HttpServlet {
	...
	public void service(HttpServletRequest request, HttpServletResponse response)
		...
		CEntityBussiness cEntityBussiness=iBllFrame.GetBussinessRunningState(cEntityEmployee);
		
		boolean bisEnter=false;
		if(cEntityBussiness!=null){
			bisEnter=true;
		}
		JSONObject outjson=new CEntityBussiness.BuildJsonObject().ToSingle(cEntityBussiness).Operation(operation).Check(bisEnter).build();
