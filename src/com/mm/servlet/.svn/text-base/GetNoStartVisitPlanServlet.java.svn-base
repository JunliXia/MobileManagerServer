...
public class GetNoStartVisitPlanServlet extends HttpServlet {
	...
	public void service(HttpServletRequest request, HttpServletResponse response)
		...
		int operation = Integer.parseInt(request
				.getParameter(MyOpcode.Operation.OPERATION));
		int EmployeeId = Integer.parseInt(request
				.getParameter(MyOpcode.Employee.EmployeeId));

		PrintWriter out = response.getWriter();
		MySpring context = MySpring.getInstance();
		IBllFrame iBllFrame = (IBllFrame) context.getContext().getBean(
				"cBllFrameImpl");

		CEntityEmployee cEntityEmployee = new CEntityEmployee.Builder()
				.EmployeeId(EmployeeId).build();
		CEntityVisitPlanArray cEntityVisitPlanArray = iBllFrame
				.GetNoStartVisitPlan(cEntityEmployee);
		...
		// JSONObject outjson=new
		// CEntityVisitPlan.BuildJsonObject().Operation(operation).Check(bisGet).MyJSONArray(cEntityVisitPlanArray.toJsonArray()).build();
